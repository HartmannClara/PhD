...
for ii=1:3
    fn1=fieldnames(events_averg.(fn{ii})); 
        for j=17:numel(fn1)
            % divide trials by 100 for the 100 shuffles
            start =1;
            Eend = (size(events_averg.(fn{ii}).(fn1{j}),3)/100);
            for k =1:100
            events_averg.shuf.(fn{ii}).(fn1{j})(k).Nr = events_averg.(fn{ii}).(fn1{j})(:,:,start:Eend);
            start = start + (size(events_averg.(fn{ii}).(fn1{j}),3)/100);
            Eend = Eend + (size(events_averg.(fn{ii}).(fn1{j}),3)/100);
            end
        end
end
...
for ii=1: numel(fn)
    fn1=fieldnames(events_averg.shuf.(fn{ii}));
        for j=1:numel(fn1)
            if j == 4 || j ==16%soc expl
                for k = 1:100
                avergTrials = nanmean(events_averg.shuf.(fn{ii}).(fn1{j})(k).Nr,3);
                avergTrialsMean = nanmean(avergTrials(10:25,:),1);
                events_averg.shuf.(fn{ii}).(fn1{j})(k).Mean = avergTrialsMean;
                end
            elseif j == 7 || j == 8 %drink eat
                for k = 1:100
                avergTrials = nanmean(events_averg.shuf.(fn{ii}).(fn1{j})(k).Nr,3);
                avergTrialsMean = nanmean(avergTrials(30:45,:),1);
                events_averg.shuf.(fn{ii}).(fn1{j})(k).Mean = avergTrialsMean;
                end
            elseif   j ==22%home
                for k = 1:100
                avergTrials = nanmean(events_averg.shuf.(fn{ii}).(fn1{j})(k).Nr,3);
                avergTrialsMean = nanmean(avergTrials(30:90,:),1);
                events_averg.shuf.(fn{ii}).(fn1{j})(k).Mean = avergTrialsMean;
                end
            elseif  j == 23 || j == 24% antic
                for k = 1:100
                avergTrials = nanmean(events_averg.shuf.(fn{ii}).(fn1{j})(k).Nr,3);
                avergTrialsMean = nanmean(avergTrials(14:29,:),1);
                events_averg.shuf.(fn{ii}).(fn1{j})(k).Mean = avergTrialsMean;
                end
            elseif j ~= 15 % without run cells
                for k = 1:100
                avergTrials = nanmean(events_averg.shuf.(fn{ii}).(fn1{j})(k).Nr,3);
                avergTrialsMean = nanmean(avergTrials(15:30,:),1);
                events_averg.shuf.(fn{ii}).(fn1{j})(k).Mean = avergTrialsMean;
                end
            end 
        end
...
fn1=fieldnames(events_averg.shufCells.g5);
    for j=1:numel(fn1)
        events_averg.shufCells.all.(fn1{j}) = cat(2,events_averg.shufCells.g5.(fn1{j}),events_averg.shufCells.g2.(fn1{j}),events_averg.shufCells.g4.(fn1{j}));
    end 
