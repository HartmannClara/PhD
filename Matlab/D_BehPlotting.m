...
%% transitions
animal ="g4";
%mat = entriesBehavior.matrix_averg;%# A 5-by-5 matrix of random values from 0 to 1
mat = entriesBehavior.matrix(:,:,3);
...
%% all trials from one cell EXITS
...
for CellID = 8:13
    ...
    subplot(2,col,CellID-st);
    plot(C1, LineWidth=1);hold on; plot((mean(C1,2)),'k' ,LineWidth=4);
    xticks([1 11 21 31 41 51 61]);xticklabels({'-3','-2','-1', '0', '1', '2', '3'});xlabel('\fontsize{12}time from event[s]');xlim([1 61]);ylim([-5 10]);
    ylabel('\fontsize{12}Ca-activity (z-scored, s.d.)');
