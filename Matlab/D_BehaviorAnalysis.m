    ...
    for j=1: numel(fn1)%day
        ...
        for k=1: numel(fn2)%session
            for s=1:size(ci_data.bsl.(fn{ii}).(fn1{j}),2)%access the data
                ...
                e_clsf = {fn(ii), fn1(j), s, char(event_type((entries_total(1)))),"s", consum};%add first entry to list
                Entry_clsf = [Entry_clsf; e_clsf];
                
                for ev=2:size(entries_total,1)
                    if event_type((entries_total(ev))) ~= event_type((entries_total(ev-1)))
                       clsf = "s" ;
                    else
                       clsf = "r" ;
                    end
                    ...
                    e_clsf = {fn(ii), fn1(j), s, char(event_type((entries_total(ev)))), clsf, consum};
                    Entry_clsf = [Entry_clsf; e_clsf];
                end 

                % classification loop to check if entry is the same as
                % entry before
                %for ev=2:size(entries_total,1)% starts at 2nd enty because first entry is always single
                %    if event_type((entries_total(ev))) ~= event_type((entries_total(ev-1)))
                %        e_clsf = {fn(ii), fn1(j), s, ev};
                %        Entry_clsf = [Entry_clsf; e_clsf];
                %    end
                %end 
