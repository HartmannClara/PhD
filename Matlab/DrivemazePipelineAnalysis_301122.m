...
% Analysis of ci_data file
% 30.11.2022
% to do:
% separate entry trials in cons and non-cons
% use latency to consume and amount consumed information
%% 
skipSessionFrames = 20; %nr of missed frames when sessions are skipped
skippedCounter = 0; %counts how many sessions are skipped bc nr of missed frames is too high
skipDrink = 0; skippedSessions = {};
        ...
        for k=1: numel(fn2)
            for s=1:size(ci_data.bsl.(fn{ii}).(fn1{j}),2)%access the data
                ...
                elseif size(ed_d_frames,1) > size(unique(ed_d_frames),1)
                    pp = [1:5 7:15]; skipDrink = skipDrink +1;
                    skipped = {fn(ii), fn1(j), s};
                    skippedSessions = cat(2,skippedSessions,skipped);  
                end    
                        ...
                        for i=1:size(trigs,1)%loop through the events of type t
                            trig=trigs(i);
                                ...
                                elseif trig>win+1 & p==4
                                    ...
                                    snips2(:,:,i)=raw_f(trig:trig+win,:); %explore activity
