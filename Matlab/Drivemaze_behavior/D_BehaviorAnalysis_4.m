...
fn=fieldnames(ci_data.Swap);
%loop through the fields
for ii=1: numel(fn)%animal
    fn1=fieldnames(ci_data.Swap.(fn{ii}));
    ...
    ed_averg_Swap=[];ef_averg_bsl=[];es_averg_bsl=[];ee_averg_bsl=[];er_averg_bsl=[];drink_averg_bsl=[];eat_averg_bsl=[];blockend_averg_bsl=[];soc_averg_bsl=[];expl_averg_bsl =[];
    ...
    EntryClsf = cell2table(Entry_clsf,"VariableNames",["animal" "day" "session" "hw" "type" "clasf" "count" "consum"]);EntryCl = EntryClsf(:,1:4);
...
edNCg2 = find(table2array(Clsf_entry.g2.ed(:,3)) == false);
edNCg5 = find(table2array(Clsf_entry.g5.ed(:,3)) == false);
edNCg4 = find(table2array(Clsf_entry.g4.ed(:,3)) == false);
efNCg2 = find(table2array(Clsf_entry.g2.ef(:,3)) == false);
efNCg5 = find(table2array(Clsf_entry.g5.ef(:,3)) == false);
efNCg4 = find(table2array(Clsf_entry.g4.ef(:,3)) == false);
...
for ii=1: numel(fn)%animal
    ...
    for jj = 1:numel(fn1) %entry type
        Singles = find(table2array(Clsf_entry.(fn{ii}).(fn1{jj})(:,1)) == "s");% indexes of single entries
        ...
        FR = find(table2array(Clsf_entry.(fn{ii}).(fn1{jj})(:,1)) == "sr");
        events_classif.(fn{ii}).(fn1{jj}).FR = events_averg.(fn{ii}).(evnt)(:,:,FR);
...
for ii=1: numel(fn)%animal
    ...
    for jj = 1:numel(fn1) %entry type
        ...
        Singles = find(table2array(Clsf_entry.(fn{ii}).(fn1{jj})(:,1)) == "s");% indexes of single entries
        if fn1{jj} == 'ee'
            evnt = char("exe_averg_bsl");
        else 
            evnt = strrep(fn1(jj),'e','ex'); evnt = char(strcat(evnt, "_averg_bsl"));
        end    
        events_classif.(fn{ii}).(evnt).singles = events_averg.(fn{ii}).(evnt)(:,:,Singles);

        Runs = events_averg.(fn{ii}).(evnt); Runs(:,:, Singles) =[];
        events_classif.(fn{ii}).(evnt).Runs =Runs;

        FR = find(table2array(Clsf_entry.(fn{ii}).(fn1{jj})(:,1)) == "sr");
        events_classif.(fn{ii}).(evnt).FR = events_averg.(fn{ii}).(evnt)(:,:,FR);
...
save("events_classif_exits.mat", "events_classif");
