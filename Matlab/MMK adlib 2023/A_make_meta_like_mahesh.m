...
%day = extractAfter(topLevelFolder,pat);
day = '2023_09_01';
day2 = strrep(day,'_','-');% get the day
chop_from=datetime([day2 ' 14:27:00.000'],'InputFormat','yyyy-MM-dd HH:mm:ss.SSS');%chop the day
...
%% saving to struct
%meta = struct with 2 fields ( exp_note and imaging)
%imaging = struct with 4 fields (stack_id events cell_f np_f)

meta2(1).exp_note = "bsl ad lib"
meta2(2).exp_note = "habitutation"

meta2(1).imaging(s).cell_f = raw_f_res;
meta2(1).imaging(s).events = events;
meta2(1).imaging(s).np_f = Fneu;
meta2(1).imaging(s).stack_id = table2cell(imaging_datasets(s,1));
