...
%% import data
topLevelFolder = "D:\Exp_2_DM_exploration\PFC-LH\g5\2022_09_23"; % or whatever, such as 'C:\Users\John\Documents\MATLAB\work'
...
ko=0;
switchFW= 20;%FED removed
imaging_datasets = folders(ko+1:36,1);
...
averg1=[];averg2=[];averg3=[];averg4=[];averg5=[];averg6=[];averg7=[];%accumulators for baseline
averg1sw=[];averg2sw=[];averg3sw=[];averg4sw=[];averg5sw=[];averg6sw=[];averg7sw=[];%accumulators for switch or FED removed/off
...
stack_path=['D:\Exp_2_DM_exploration\PFC-LH\g5\2022_09_23\' stack{1, 1} '\My_V4_Miniscope\Results.csv'];
list_path=['D:\Exp_2_DM_exploration\PFC-LH\g5\2022_09_23\' stack{1, 1} '\My_V4_Miniscope\timeStamps.csv'];
...
%% chop
chop_from=datetime('2022-09-23 11:00:00.000','InputFormat','yyyy-MM-dd HH:mm:ss.SSS');%chop the day
...
%% averg plots
f4=figure;% compare feed entry with and wo FED
figure(f4);hold on;
        subplot(1,2,1),imagesc([-win:win],[1:size(averg2_mean,2)],averg2_mean');
        title(['\fontsize{10}enter feed',' trials:',num2str(size(averg2,3))]);
        subplot(1,2,2),imagesc([-win:win],[1:size(averg2sw_mean,2)],averg2sw_mean');
        title(['\fontsize{10}enter feed (no FED)',' trials:',num2str(size(averg2sw,3))]);

% Rasterplots for individual cells to show temporal changes

cell8Frm = reshape(averg2sw(:,8,:), size(averg2sw,1),size(averg2sw,3));
f6 = figure;
figure(f6); hold on;
    subplot(1,1,1),imagesc([-win:win],[1:size(cell8Frm,2)],cell8Frm');
    title(['\fontsize{10}enter feed cell8 FED removed']);
