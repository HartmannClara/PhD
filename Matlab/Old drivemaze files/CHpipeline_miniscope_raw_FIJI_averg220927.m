...
%% import data
topLevelFolder = "E:\Exp_2_DM_exploration\PFC-LH\g5\2022_09_27"; % or whatever, such as 'C:\Users\John\Documents\MATLAB\work'
...
ko=0;
imaging_datasets = folders(ko+1:19,1);
...
averg1=[];averg2=[];averg3=[];averg4=[];averg5=[];averg6=[];averg7=[];averg8=[];
...
stack_path=['E:\Exp_2_DM_exploration\PFC-LH\g5\2022_09_27\' stack{1, 1} '\My_V4_Miniscope\Results.csv'];
list_path=['E:\Exp_2_DM_exploration\PFC-LH\g5\2022_09_27\' stack{1, 1} '\My_V4_Miniscope\timeStamps.csv'];
events_path=['E:\Exp_2_DM_exploration\PFC-LH\g5\3354903011_events.csv'];
frame_list=import_framelist(list_path);
events=import_events(events_path);
raw_f=import_raw(stack_path);%zscored in function

%% chop
chop_from=datetime('2022-09-27 13:00:00.000','InputFormat','yyyy-MM-dd HH:mm:ss.SSS');%chop the day
chop_to=datetime('2022-09-27 20:00:00.000','InputFormat','yyyy-MM-dd HH:mm:ss.SSS');
...
figure(f4);hold on;
        subplot(1,8,1),imagesc([-win:win],[1:size(averg1_mean,2)],averg1_mean');
        title(['\fontsize{10}enter drink',' trials:',num2str(size(averg1,3))]);

        subplot(1,8,2),imagesc([-win:win],[1:size(averg2_mean,2)],averg2_mean');
        title(['\fontsize{10}enter feed',' trials:',num2str(size(averg2,3))]);

        subplot(1,8,3),imagesc([-win:win],[1:size(averg3_mean,2)],averg3_mean');
        title(['\fontsize{10}enter social',' trials:',num2str(size(averg3,3))]);

        subplot(1,8,4),imagesc([-win:win],[1:size(averg4_mean,2)],averg4_mean');
        title(['\fontsize{10}enter explore',' trials:',num2str(size(averg4,3))]);

        subplot(1,8,5),imagesc([-win:win],[1:size(averg5_mean,2)],averg5_mean');
        title(['\fontsize{10}enter run',' trials:',num2str(size(averg5,3))]);

        subplot(1,8,6),imagesc([-win:win],[1:size(averg6_mean,2)],averg6_mean');
        title(['\fontsize{10}drink',' trials:',num2str(size(averg6,3))]);

        subplot(1,8,7),imagesc([-win:win],[1:size(averg7_mean,2)],averg7_mean');
        ...
        title(['\fontsize{10}block_end',' trials:',num2str(size(averg8,3))]);
        clim([-5 5])
