%clear all; close all;
clearvars -except averg1 averg2 averg3 averg4 averg5 averg6 averg7; close all;% if days are run in sequence keep variables
set(0,'defaultAxesFontSize',18);
%% import data
topLevelFolder = "D:\Exp_2_DM_exploration\PFC-LH\g5\2022_09_12"; % or whatever, such as 'C:\Users\John\Documents\MATLAB\work'
...
ko=0;
switchFW= 12;
imaging_datasets = folders(ko+1:24,1);
%imaging_datasets2 = folders(switchFW:24,1);
 %to add to linear imaging starts index, if imaging files start at 20, ko=19
bad=15; %bad imaging dataset to be excluded later
%imaging_datasets=table(['11_25_35'; '11_34_12']);%;'14_48_41';'14_56_33']);%;'';'';''
%averg1=[];averg2=[];averg3=[];averg4=[];averg5=[];averg6=[];averg7=[];
averg1sw=[];averg2sw=[];averg3sw=[];averg4sw=[];averg5sw=[];averg6sw=[];averg7sw=[];
for s=1:size(imaging_datasets,1)%:5%:3%change which imaging sets here
    ...
    stack_path=['D:\Exp_2_DM_exploration\PFC-LH\g5\2022_09_12\' stack{1, 1} '\My_V4_Miniscope\Results.csv'];
    list_path=['D:\Exp_2_DM_exploration\PFC-LH\g5\2022_09_12\' stack{1, 1} '\My_V4_Miniscope\timeStamps.csv'];
    ...
    %% chop
    chop_from=datetime('2022-09-12 10:00:00.000','InputFormat','yyyy-MM-dd HH:mm:ss.SSS');%chop the day
    chop_to=datetime('2022-09-12 18:00:00.000','InputFormat','yyyy-MM-dd HH:mm:ss.SSS');
    ...
    for i=1:size(imaging_ends,1)
        ...
        imaging_sets = [imaging_sets; events.Date_Time(imaging_starts(i))];%to check if it matches with folders
    end
    ...
    %% imaging data
    %figure;stackedplot(raw_f);title(stack);
    size(raw_f);
    win=100;%window size in frames 10f/s
...
%                         elseif p==8
%                             averg8=cat(3,averg8,snips(:,:,i));
%                             %averg8_mean= nanmean(averg8,3);    
                        end
                    end
                    averg1_mean= nanmean(averg1,3);
                    averg2_mean= nanmean(averg2,3);
            ...
            if size(trigs,1)==0
                ...
                for i=1:size(trigs,1)
                    ...
                    if trig>win+1 %& s ~= bad %excludes bad datasets
                        ...
                        if q==1
                            averg1sw=cat(3,averg1sw,snips(:,:,i));
                            %averg1_mean= nanmean(averg1,3);
                        elseif q==2
                            averg2sw=cat(3,averg2sw,snips(:,:,i));
                            %averg2_mean= nanmean(averg2,3);
                        elseif q==3
                            averg3sw=cat(3,averg3sw,snips(:,:,i));
                            %averg3_mean= nanmean(averg3,3);
                        elseif q==4
                            averg4sw=cat(3,averg4sw,snips(:,:,i));
                            %averg4_mean= nanmean(averg4,3);
                        elseif q==5
                            averg5sw=cat(3,averg5sw,snips(:,:,i));
                            %averg5_mean= nanmean(averg5,3);
                        elseif q==6
                            averg6sw=cat(3,averg6sw,snips(:,:,i));
                            %averg6_mean= nanmean(averg6,3);
                        elseif q==7
                            averg7sw=cat(3,averg7sw,snips(:,:,i));
                            %averg7_mean= nanmean(averg7,3);
%                         elseif q==8
%                             averg8=cat(3,averg8,snips(:,:,i));
%                             %averg8_mean= nanmean(averg8,3);    
                    ...
                    averg1sw_mean= nanmean(averg1sw,3);
                    averg2sw_mean= nanmean(averg2sw,3);
                    averg3sw_mean= nanmean(averg3sw,3);
                    averg4sw_mean= nanmean(averg4sw,3);
                    averg5sw_mean= nanmean(averg5sw,3);
                    averg6sw_mean= nanmean(averg6sw,3);
                    averg7sw_mean= nanmean(averg7sw,3);
                    %averg8sw_mean= nanmean(averg8sw,3);
                end
        %         response_mean=nanmean(snips_bslined,3);
        %        response_mean=nanmean(snips,3); %makes individual plots per session according to nr in imaging sessions
        %        figure(f3);hold on;
...
% %subtracting explore activity
% averg1_mean= averg1_mean - averg4_mean;
% averg2_mean= averg2_mean - averg4_mean;
% averg1sw_mean= averg1sw_mean - averg4sw_mean;
% averg2sw_mean= averg2sw_mean - averg4sw_mean;
% averg3_mean= averg3_mean - averg4_mean;
% averg4_mean= averg4_mean;
% averg5_mean= averg5_mean - averg4_mean;
% averg6_mean= averg6_mean - averg4_mean;
% averg7_mean= averg7_mean - averg4_mean;
% averg6sw_mean= averg6sw_mean - averg4sw_mean;
% averg7sw_mean= averg7sw_mean - averg4sw_mean;


% averg8_mean= averg8_mean - averg4_mean;
...
%         subplot(1,8,8),imagesc([-win:win],[1:size(averg8_mean,2)],averg8_mean');
%         title(['\fontsize{10}run',' trials:',num2str(size(averg8,3))]);


% f5=figure;% averages across all switch sessions
% figure(f5);hold on;
%         subplot(1,7,1),imagesc([-win:win],[1:size(averg1sw_mean,2)],averg1sw_mean');
%         title(['\fontsize{10}enter drink',' trials:',num2str(size(averg1sw,3))]);
% 
%         subplot(1,7,2),imagesc([-win:win],[1:size(averg2sw_mean,2)],averg2sw_mean');
%         title(['\fontsize{10}enter feed',' trials:',num2str(size(averg2sw,3))]);
% 
%         subplot(1,7,3),imagesc([-win:win],[1:size(averg3sw_mean,2)],averg3sw_mean');
%         title(['\fontsize{10}enter social',' trials:',num2str(size(averg3sw,3))]);
% 
%         subplot(1,7,4),imagesc([-win:win],[1:size(averg4sw_mean,2)],averg4sw_mean');
%         title(['\fontsize{10}enter explore',' trials:',num2str(size(averg4sw,3))]);
% 
%         subplot(1,7,5),imagesc([-win:win],[1:size(averg5sw_mean,2)],averg5sw_mean');
%         title(['\fontsize{10}enter run',' trials:',num2str(size(averg5sw,3))]);
% 
%         subplot(1,7,6),imagesc([-win:win],[1:size(averg6sw_mean,2)],averg6sw_mean');
%         title(['\fontsize{10}drink',' trials:',num2str(size(averg6sw,3))]);
% 
%         subplot(1,7,7),imagesc([-win:win],[1:size(averg7sw_mean,2)],averg7sw_mean');
%         title(['\fontsize{10}retrieve pellet',' trials:',num2str(size(averg7sw,3))]);


% f4=figure;% changes in switch
% figure(f4);hold on;
% 
%         subplot(1,6,1),imagesc([-win:win],[1:size(averg3_mean,2)],averg3_mean');
%         title(['\fontsize{10}enter social',' trials:',num2str(size(averg3,3))]);
% 
%         subplot(1,6,2),imagesc([-win:win],[1:size(averg3sw_mean,2)],averg3sw_mean');
%         title(['\fontsize{10}enter social(switch)',' trials:',num2str(size(averg3sw,3))]);
% 
%         subplot(1,6,3),imagesc([-win:win],[1:size(averg4_mean,2)],averg4_mean');
%         title(['\fontsize{10}enter explore',' trials:',num2str(size(averg4,3))]);
% 
%         subplot(1,6,4),imagesc([-win:win],[1:size(averg4sw_mean,2)],averg4sw_mean');
%         title(['\fontsize{10}enter explore(switch)',' trials:',num2str(size(averg4sw,3))]);
% 
%         subplot(1,6,5),imagesc([-win:win],[1:size(averg5_mean,2)],averg5_mean');
%         title(['\fontsize{10}enter run',' trials:',num2str(size(averg5,3))]);
% 
%         subplot(1,6,6),imagesc([-win:win],[1:size(averg5sw_mean,2)],averg5sw_mean');
%         title(['\fontsize{10}enter run (switch)',' trials:',num2str(size(averg5sw,3))]);
% 

% does switch profile remain the same over time?
% for fi=1:size()a
%   response_mean=nanmean(snips,3); %makes individual plots per session according to nr in imaging sessions
%   figure(f3);hold on;
%   subplot(m,n,p+s*7-7),imagesc([-win:win],[1:size(response_mean,2)],response_mean');
%   title(['\fontsize{10}',tt,' trials:',num2str(size(snips,3))]);
% 1averg1sw_mean= nanmean(averg1sw,3);
% averg2sw_mean= nanmean(averg2sw,3);
% averg3sw_mean= nanmean(averg3sw,3);
% averg4sw_mean= nanmean(averg4sw,3);
% averg5sw_mean= nanmean(averg5sw,3);
...
% f4=figure;% comparison switch bsl
% figure(f4);hold on;
%         subplot(1,4,1),imagesc([-win:win],[1:size(averg1_mean,2)],averg1_mean');
%         title(['\fontsize{10}enter drink',' trials:',num2str(size(averg1,3))]);
% 
%         subplot(1,4,2),imagesc([-win:win],[1:size(averg2sw_mean,2)],averg2sw_mean');
%         title(['\fontsize{10}enter feed(switch)',' trials:',num2str(size(averg2sw,3))]);
% 
%         subplot(1,4,3),imagesc([-win:win],[1:size(averg2_mean,2)],averg2_mean');
%         title(['\fontsize{10}enter feed',' trials:',num2str(size(averg2,3))]);
% 
%         subplot(1,4,4),imagesc([-win:win],[1:size(averg1sw_mean,2)],averg1sw_mean');
%         title(['\fontsize{10}enter drink(switch)',' trials:',num2str(size(averg1sw,3))]);

        
        
% f4=figure;% comparison switch bsl
% figure(f4);hold on;
%         subplot(1,4,1),imagesc([-win:win],[1:size(averg6_mean,2)],averg6_mean');
%         title(['\fontsize{10}drink',' trials:',num2str(size(averg6,3))]);
% 
%         subplot(1,4,3),imagesc([-win:win],[1:size(averg7_mean,2)],averg7_mean');
%         title(['\fontsize{10}retrieve pellet',' trials:',num2str(size(averg7,3))]);
% 
%         subplot(1,4,2),imagesc([-win:win],[1:size(averg6sw_mean,2)],averg6sw_mean');
%         title(['\fontsize{10}drink(switch)',' trials:',num2str(size(averg6sw,3))]);
% 
%         subplot(1,4,4),imagesc([-win:win],[1:size(averg7sw_mean,2)],averg7sw_mean');
%         title(['\fontsize{10}retrieve pellet(switch)',' trials:',num2str(size(averg7sw,3))]);
