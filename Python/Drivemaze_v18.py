#drivemaze v18
# with variable adlib interval between 1 and 2s and pellet delivery event
#usb openscale, email alarm, swap, off and adlib options
#with recording limit
#2023 Sep 1st
...
import smbus2
import yagmail
experiment = "adlib"#"adlib" #"habituation"#"FEDoff"#"baseline"#"FED removed""FWswap" "FEDon"
FEDonFLAG = True #change for turning FED on or off
Adlib_Flag = True #change for adlib food access
Adlib_var = True #change for variable adlib interval otherwise fixed adlib interval
Swap_Flag = False # change for swapping food and water pod
Limit_img_Flag = True #change for limiting imaging time
pi = pigpio.pi() 
...
#subjects = ["8500188177" ,"3354909574", "3354916686", "335490250239", "8500142131"]#L,nm, R, L NM
subjects = ["3354902615", "1251667485196"]#stick, L
#general variables
...
img_limit = 24000 #40 min for experimets #12000 #frames 20 min maximum nr before stopping imaging
...
water_interval=550 # ms minimal between water drops - 550 same as FED interval
food_interval=550
water_flag=False
...
#             animaltag=tag
        #if tag==335490249236 or tag==8500188177 or tag==3354909574 or tag==3354916686 or tag==335490250239 or tag == 8500142131:
        if tag == 3354902615 or tag == 1251667485196:
            animal_found_flag=True 
            #print("\nfound animal: ")
            #print(tag)
            animaltag=tag
...
#             print("no movement")
            try:
                 ...
                 frame_counter=fc.tally()
                 tick = pi.get_current_tick()
                 save.append_event("", "", "no_movement_email", animaltag,frame_counter,tick,experiment)
        ...
        if not stop_imaging_flag and Limit_img_Flag and frame_counter > img_limit:
            pi.write(start_scope_port,0) #stop acquisition
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
            save.append_event("", "", "imaging_stop_20", animaltag,frame_counter,tick,experiment)    
            stop_imaging_flag=True
            img_break_timer = int(round(time.time() * 1000))#start break timer while animal is still in maze
            print("20 min recorded -- break timer started")
            mode=4 
...
#             pi.write(FED_in,0)
        ...
        if not pi.read(beaml[2]) and not pod_entry_flag and Swap_Flag: #unit1 is water
            email_timer = int(round(time.time() * 1000))
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
        ...
        if food_flag and pi.read(FED_out): #rec delay to feed
            ...
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
            save.append_event(pellets, food_delay, "retrieve_pellet", animaltag,frame_counter,tick,experiment) 
            food_timer=int(round(time.time() * 1000))
        ...
        #variable interval
        if Adlib_var and not Fint:
           #random number between 1 and 2s
           food_interval=(np.random.randint(100,200))*10
           Fint=True

        #adlib option if flag is set
        if Adlib_Flag and not food_flag and pod_entry_flag and int(round(time.time() * 1000))-food_timer>food_interval: #next pellet after 500ms FED_out TTL is down
            pi.write(FED_in,1) #next pellet
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
            save.append_event(pellets, food_delay, "give_pellet", animaltag,frame_counter,tick,experiment)
            food_flag=True
            Fint=False
            food_timer=int(round(time.time() * 1000))
...
#             pi.write(FED_in,0)
            ...
            food_delay=[]
            unit1_counter= unit1_counter +1
        #adlib water if Adlib_Flag is true  
        if Adlib_Flag and rec_licks_flag:
            #record licking
            if pi.read(lick_in_port):
                ...
                if water_flag:
                    ...
                    frame_counter=fc.tally()
                    tick = pi.get_current_tick()
                    save.append_event(drop,"", "drink", animaltag,frame_counter,tick,experiment)                 
                    pi.write(lick_out_port,1) # start giving water
                    water_dose_timer=int(round(time.time() * 1000))
                    water_flag=False
                    water_stop_dose_flag=True
            if water_stop_dose_flag and millis-water_dose_timer>water_duration:# giving of "first" water drop
                pi.write(lick_out_port,0)
                water_stop_dose_flag=False
                frame_counter=fc.tally()
                tick = pi.get_current_tick()
                save.append_event(licks, drink_delay, "drop_stats", animaltag,frame_counter,tick,experiment)
                #print("drink delay was")
                #print(drink_delay)   

            if Adlib_var and not waterInt:
                water_interval=(np.random.randint(100,200))*10
                waterInt=True
         
            if not water_flag and not water_stop_dose_flag and millis-water_dose_timer>water_interval:#giving of next water drop, controls water flag to give next drop via water interval
                lick_timer=int(round(time.time() * 1000))
                water_flag=True
                water_stop_dose_flag=False
                licks=0
                ...
                frame_counter=fc.tally()
                tick = pi.get_current_tick()
                save.append_event(drop,"", "water_available", animaltag,frame_counter,tick,experiment)

        #normal water delivery        
        if not Adlib_Flag and rec_licks_flag:
            #record licking
            if pi.read(lick_in_port):
                licks=licks+1
                if water_flag:
...
#                 print("m5")
    ...
    if mode==5: #wait for exit or entry of other
        if millis-scale_timer>scale_interval_scan:
            w=read_scale()
            scale_timer=int(round(time.time() * 1000))
        if w<6 and pi.read(beaml[1]):
            if exit_safety_timer_flag:
#                 print("putative exit")
                exit_safety_timer=int(round(time.time() * 1000))
                exit_safety_timer_flag=False
                w=read_scale()
            if not exit_safety_timer_flag and w<6 and pi.read(beaml[0]) and int(round(time.time() * 1000))-exit_safety_timer>safety_delay:
                close_door(1)
...
#                 print("m6")
    ...
    if mode==6: #wait for nest choice time out
        if not stop_imaging_flag and millis-acquisition_end_timer>acquisition_end_timeout :
            pi.write(start_scope_port,0) #stop acquisition
            img_break_timer = int(round(time.time() * 1000))#timer for break length
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
            ...
            stop_imaging_flag=True
            print("nest timeout:")  
            print(((frame_counter/10)/60), "minutes")
            
        if Limit_img_Flag and stop_imaging_flag and int(round(time.time() * 1000)) - img_break_timer > (frame_counter*100):   #animal put on break for same time as imaging
            print("Pellets:")
            print(pellet_counter)
            print("Food pod entries:")
            print(unit1_counter)
        ...
        if frame_counter < 10  and int(round(time.time() * 1000))-nest_timer>nest_timeout: # if imaging is not limited by time, then wait for preset nest timeout
            frame_counter=0
            for x in range(np.size(subjects)):
                animaltag=subjects[x]
                save.append_event("", "", "block_available", animaltag,frame_counter,tick,experiment)
