# -*- coding: utf-8 -*-
"""
Created on Wed Mar 23 15:01:51 2022
...
for x in daylist:
    filepath = folderpath + x +"/" 
    filelist =os.listdir(filepath) 
    files = []
    for x in filelist:
        files.append(x + '/My_V4_Miniscope')
    ...
    for x in files:
        ...
        dataFilePrefix = ''
        startingFileNum = 0
        #framesPerFile = 10000 # This is the default setting for the Miniscope software. If you changed it in software change it here too.
        # -------------------------------
        
        # Grab frames per file from metadate
        meta = open(dataDir + '/metaData.json')
        meta = json.load(meta)
        framesPerFile = meta["framesPerFile"]
        ...
        #%%    
        # Run through avi files and generate mean fft (fast Fourier transform)
        ...
        # Values users can modify:
        frameStep = 10 # This will speed up the playback
        # -----------------------
        ...
        # Values users can modify:
        fs = 10 # TODO: Should get this from timestamp file
        cutoff = 3.0
        # -----------------------
        ...
        while (path.exists(dataDir + dataFilePrefix + "{:.0f}.avi".format(fileNum)) and running is True):
            ...
            if (mode is "save"):
                ...
                #ogfile = dataDir + dataFilePrefix + "{:.0f}.avi".format(fileNum)
                #os.remove(ogfile)
